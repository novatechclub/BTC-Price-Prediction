...
from utils.clients.bitmex_client import NTCTapBitmexClient
...
PRELOADED_FILES = Path("app_assets/preloaded_assets/bitmex_preloaded_datasets")
...
def load_bitmex_view():
    ...
    if load_mode == "📦 Preloaded Dataset" and load_pre:
        if st.session_state.get("last_loaded_file_bitmex") != selected_file:
            ...
            client.last_df    = df
            auditor = NTCTapClientAudit.from_client(df, _client=client, source="bitmex", verbose=False)
    ...
    if "bitmex_df" in st.session_state and "bitmex_auditor" in st.session_state:
        ...
        with fmt_col:
            fmt = st.radio("Select File-Format", ["CSV","Parquet"], horizontal=True, label_visibility="collapsed", key="bitmex_export_format")
        with dl_col:
