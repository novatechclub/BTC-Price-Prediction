...
import pandas as pd
from utils.clients.augmento_datawrangler import NTCTapDataWrangler
from pathlib import Path
import io

PRELOADED_FILES_AUGMENTO = Path("app_assets/preloaded_assets/augmento_preloaded_datasets")
PRELOADED_FILES_BITMEX = Path("app_assets/preloaded_assets/augmento_preloaded_datasets")


def load_wrangler_view():
    # Initialize or load wrangler so sidebar actions have access
    if "wrangler" not in st.session_state:
        st.session_state.wrangler = NTCTapDataWrangler()
    ...
    pre_paths = sorted(PRELOADED_FILES_AUGMENTO.glob("*.*"))
    bitmex_paths = sorted(PRELOADED_FILES_BITMEX.glob("*.*"))
